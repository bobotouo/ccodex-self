"""Proof-of-Work challenge solver for ChatGPT's sentinel endpoint."""
...
def parse_pow_resources(html_content: str) -> tuple[list[str], str]:
    """Extract PoW script URLs and data-build from chatgpt.com HTML."""
...
def _pow_generate(seed: str, difficulty: str, config: list[Any], limit: int = 500000) -> tuple[str, bool]:
    """Solve PoW challenge via SHA3-512 brute force."""
    ...
    for i in range(limit):
        ...
        encoded = _b64encode(final_json)
        digest = hashlib.sha3_512(seed_bytes + encoded).digest()
        if digest[:diff_len] <= target:
            return encoded.decode(), True
    fallback = "wQ8Lk5FbGpA2NcR9dShT6gYjU7VxZ4D" + _b64encode(f'"{seed}"'.encode()).decode()
...
def build_legacy_requirements_token(
    ...
    """Generate a low-difficulty requirements token for the sentinel endpoint."""
...
def build_proof_token(
    ...
    """Generate a proof token with the given difficulty."""
    config = _build_pow_config(user_agent, script_sources=script_sources, data_build=data_build)
